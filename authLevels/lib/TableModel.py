...
class TableModel(QAbstractTableModel):
    def __init__(self, path, search, parent=None):
    ...
    def data(self, index, role):
        ...
        if role == Qt.EditRole:
            return QVariant(self.items[row]['plvl'])
    ...
    def setData(self, index, value, role):
        if index.column() == 1:
            if self.items[index.row()]['plvl'] != value:
                self.items[index.row()]['changed'] = True
                self.changes.append({'row' : index.row(), 'before' : self.items[index.row()]['plvl'], 'after' : value})
    ...
    def save(self,path=None):
        ...
        for change in self.changes:
            f.write(self.items[change['row']]['tag'] + " from " + change['before'] + " to " + change['after'] + '\n')
        f.close()
    ...
    def undo(self):
        ...
        self.items[change['row']]['plvl'] = change['before']
        self.items[change['row']]['changed'] = False

        for j in range(10):
            for prop in self.items[change['row']]['element'].iter("ExpProps_"+str(j)):
                    var = prop.findall("Name")[0].text
                    find = var.find("Passwordlevel")
                    if find != -1:
                        prop.findall("ExpPropValue")[0].text = "<Passwordlevel>"+change['before']+"</Passwordlevel>"
